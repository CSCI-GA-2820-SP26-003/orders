...
class TestYourResourceService(TestCase):
    ...
    def test_query_orders_by_status_cancelled_alias(self):
        ...
        canceled_orders = [o for o in orders if o.status.value == "CANCELED"]
        if not canceled_orders:
            self.skipTest("No canceled orders generated by factory")
        ...
        data = resp.get_json()
        self.assertEqual(len(data), len(canceled_orders))
        for order in data:
